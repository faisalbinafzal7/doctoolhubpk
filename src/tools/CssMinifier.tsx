...
import { Upload, X, Copy, Info, Zap, Shield, FileText, Check, RotateCcw, Download, Layout } from 'lucide-react';
...
export default function CssMinifier() {
  ...
  const [status, setStatus] = React.useState<{ message: string; type: 'info' | 'success' | 'error' }>({
    message: 'Paste CSS code, select options, and click Minify CSS.',
    type: 'info'
  });
  ...
  const stats = React.useMemo(() => {
    ...
    return {
      ...
      savedPercent: saved + '%',
      linesRemoved: input ? originalLines - minifiedLines : 0
    };
  }, [input, output]);
  ...
  const minifyCSS = React.useCallback(() => {
    if (!input) {
      setStatus({ message: '⚠️ No CSS code to minify.', type: 'error' });
      return;
    }
    ...
    setStatus({ 
      message: `✅ CSS minified successfully! Reduced from ${originalSizeKB} KB to ${minifiedSizeKB} KB.`, 
      type: 'success' 
    });
  }, [input, options]);
  ...
  const copyToClipboard = () => {
    if (!output) {
      setStatus({ message: '⚠️ No minified CSS to copy.', type: 'error' });
      return;
    }
    navigator.clipboard.writeText(output).then(() => {
      setStatus({ message: '✅ Minified CSS copied to clipboard!', type: 'success' });
      setTimeout(() => setStatus({ message: 'Ready. Paste CSS and click Minify.', type: 'info' }), 2000);
    });
  };

  const downloadCSS = () => {
    if (!output) {
      setStatus({ message: '⚠️ No minified CSS to download.', type: 'error' });
      return;
    }
    ...
    setStatus({ message: '✅ Minified CSS downloaded successfully!', type: 'success' });
  };
    ...
    <div className="space-y-8">
      <div className="bg-white premium-card rounded-3xl border border-gray-200/70 p-6 md:p-8 mb-12 shadow-sm w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-800">CSS Minifier</h2>
            <p className="text-sm text-gray-500 mt-1">Remove unnecessary spaces, line breaks, and comments to reduce CSS file size</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <OptionCard 
            ...
            description="Collapses multiple spaces to single space"
            checked={options.removeExtraSpaces}
            onChange={(checked) => setOptions(prev => ({ ...prev, removeExtraSpaces: checked }))}
          />
          <OptionCard 
            id="removeLineBreaks"
            label="Remove line breaks"
            description="Converts CSS to a single line"
            checked={options.removeLineBreaks}
            onChange={(checked) => setOptions(prev => ({ ...prev, removeLineBreaks: checked }))}
          />
        </div>
        ...
        <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-gray-600">
              <strong className="font-semibold text-gray-800">About CSS Minification:</strong> Minifying CSS removes unnecessary characters without changing functionality. This reduces file size, improves page load times, and saves bandwidth. The original CSS structure and styling remain intact.
            </div>
          </div>
        </div>
      ...
      <div className="bg-white py-10 border-t border-gray-100 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <FeatureCard 
            ...
            description="Reduce CSS file size in milliseconds."
          />
          <FeatureCard 
            icon={<Layout className="h-6 w-6 text-[#0274be]" />}
            title="Customizable options"
            description="Choose what to remove: comments, spaces, line breaks."
          />
          <FeatureCard 
            icon={<Shield className="h-6 w-6 text-[#0274be]" />}
            title="100% secure"
            description="No upload — your CSS stays in your browser."
          />
          <FeatureCard 
            icon={<Download className="h-6 w-6 text-[#0274be]" />}
            title="Download & copy"
            description="Save minified CSS or copy to clipboard."
          />
        </div>
      </div>
    </div>

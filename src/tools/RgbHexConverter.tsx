...
export default function RgbHexConverter() {
  ...
  const [status, setStatus] = useState<{ message: string; type: 'info' | 'success' | 'error' }>({
    message: 'Enter RGB values or HEX code to convert colors.',
    type: 'info'
  });
  ...
  const showStatus = (message: string, type: 'info' | 'success' | 'error' = 'info') => {
    ...
    if (type === 'success') {
      setTimeout(() => {
        setStatus({ message: 'Ready. Enter RGB or HEX values to convert.', type: 'info' });
      }, 3000);
    }
  };
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-xl font-bold text-slate-800">RGB to HEX Converter</h2>
            <p className="text-sm text-slate-500 mt-1">Convert RGB color values to HEX and vice versa</p>
          </div>
        ...
        <div className="bg-sky-50/50 p-5 rounded-2xl border border-sky-100">
          <div className="flex items-start gap-4">
            ...
            <div className="text-sm text-sky-800 leading-relaxed">
              <strong className="font-bold block mb-1">About RGB and HEX:</strong>
              RGB (Red, Green, Blue) uses values from 0-255 for each channel. HEX (hexadecimal) is a 6-digit code representing RGB values. Both represent the same color, just different formats.
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { icon: Zap, title: "Instant conversion", desc: "Convert RGB to HEX and vice versa instantly." },

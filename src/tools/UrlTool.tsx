...
import { Upload, X, Copy, ArrowLeftRight, Check, Info, Zap, Shield, Layout, FileText, Link, Globe, Search } from 'lucide-react';

export default function UrlTool() {
  const [input, setInput] = React.useState('');
  const [output, setOutput] = React.useState('');
  const [status, setStatus] = React.useState<{ message: string; type: 'info' | 'success' | 'error' }>({
    message: 'Enter text or URL, then click Encode or Decode.',
    type: 'info'
  });
  ...
  const encodeURL = () => {
    ...
    try {
      ...
      setOutput(encoded);
      setStatus({ message: '✅ URL encoded successfully!', type: 'success' });
    } catch (e) {
  ...
  const decodeURL = () => {
    if (!input) {
      setStatus({ message: '⚠️ No text to decode.', type: 'error' });
      return;
    }
    try {
      const decoded = decodeURIComponent(input);
      setOutput(decoded);
      setStatus({ message: '✅ URL decoded successfully!', type: 'success' });
    } catch (e) {
      setStatus({ message: `❌ Decoding failed: Invalid URL encoding. ${(e as Error).message}`, type: 'error' });
    }
  };
    ...
    <div className="space-y-8">
      <div className="bg-white premium-card rounded-3xl border border-gray-200/70 p-6 md:p-8 mb-12 shadow-sm w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-800">URL Encoder / Decoder</h2>
            <p className="text-sm text-gray-500 mt-1">Encode special characters to URL-safe format or decode back to original</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-2 flex items-center gap-2">
              <Link className="h-4 w-4 text-[#0274be]" />
              Input Text / URL
            </label>
            <textarea 
              value={input}
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 block mb-2 flex items-center gap-2">
              <Globe className="h-4 w-4 text-[#0274be]" />
      ...
      <div className="bg-white py-10 border-t border-gray-100 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <FeatureCard 
            ...
            description="Encode or decode URLs with one click."
          />
          <FeatureCard 
            icon={<Globe className="h-6 w-6 text-[#0274be]" />}
            title="Full URL support"
            description="Handles all special characters and Unicode."
          />
          <FeatureCard 
